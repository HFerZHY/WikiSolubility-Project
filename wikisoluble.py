...
C = tk.Canvas(frame, bg = '#FFFFFF', width = 450, height = 480, highlightbackground = '#FFFFFF', highlightcolor = '#FFFFFF', bd = 1)
...
def realdraw(imformlist: list, brightcolor: str, darkcolor:str):
	...
	for i in range(len(points)):
		...
		if i != 0:
			if isdark:
				C.create_line(lastx, lasty, x, y, width = 3, tag = 'line', fill = darkcolor)
			else:
				C.create_line(lastx, lasty, x, y, width = 3, tag = 'line', fill = brightcolor)
		if isdark:
			...
			C.create_oval(x-3, y-3, x+3, y+3, fill = '#332255', width = 0)
			C.create_text(x+20, y, text = str(points[i][0]) + '°C\n' + str(points[i][1]), fill = '#40709F', font = ft3, tag = 'text')
		lastx = x
		lasty = y
	if ifdec:
		x = dectemp * 3.75 + 25
		...
		C.create_text(x+45, y, text = sublist[m].formula +'\nDecomposes\nAt ' + str(dectemp) + '°C', fill = '#FF0000', font = ft3, tag = 'text')
	if ifmis:
		x = mistemp * 3.75 + 25
		y = 25
		C.create_oval(x-3, y-3, x+3, y+3, fill = '#0000FF', width = 0)
		C.create_text(x+45, y, text = sublist[m].formula + '\nMiscibles\n At ' + str(mistemp) + '°C', fill = '#0000FF', font = ft3, tag = 'text')
	C.lift('text')
	C.lower('line')
	window.update()
...
def cvstitle(m1, m2):
	if m2 == -1:
		if isdark:
			C.create_text(225, 460, text = 'Solubility Fold Line Diagram of\n%s (%s)' % ((sublist[m1].name).title(), sublist[m1].formula), font = ft2, fill = '#FFFFFF')
		else:
			C.create_text(225, 460, text = 'Solubility Fold Line Diagram of\n%s (%s)' % ((sublist[m1].name).title(), sublist[m1].formula), font = ft2, fill = '#0F102E')
	else:
		if isdark:
			C.create_text(225, 440, text = 'Solubility Comparison Fold Line Diagram of\n%s (%s) and\n%s (%s)' % ((sublist[m1].name).title(), sublist[m1].formula, (sublist[m2].name).title(), sublist[m2].formula), font = ft2, fill = '#FFFFFF')
		else:
			C.create_text(225, 440, text = 'Solubility Comparison Fold Line Diagram of\n%s (%s) and\n%s (%s)' % ((sublist[m1].name).title(), sublist[m1].formula, (sublist[m2].name).title(), sublist[m2].formula), font = ft2, fill = '#0F102E')
	window.update()
...
def contradraw(m1, m2):
	...
	if list1[2] < list2[2]:
		...
		list1[2] = list2[2]
	realdraw(list1, '#0F104E', '#FF6699')
	realdraw(list2, '#4E100F', '#9966FF')	
	cvstitle(m1, m2)
	if isdark:
		C.create_text(225, 407, text = sublist[m1].formula, fill = '#FF6699', font = ft3)
		C.create_text(225, 395, text = sublist[m2].formula, fill = '#9966FF', font = ft3)
	else:
		C.create_text(225, 407, text = sublist[m1].formula, fill = '#0F104E', font = ft3)
		C.create_text(225, 395, text = sublist[m2].formula, fill = '#4E100F', font = ft3)
...
T = tk.Text(frame, width = 80, height = 25, font = ft2, bg = '#CDD7E2')
